...
public class RobotContainer {
    ...
    Command PathfindToScoringBlue = AutoBuilder.pathfindToPose(
        ...
        new PathConstraints(4.0, 4.0, Units.degreesToRadians(0), Units.degreesToRadians(540)),
        0,
        0
    );
    ...
    private void configureButtonBindings() {
        ...
        auxSquare.onTrue(new InstantCommand(() -> { // [Hold] Outtake
            endEffector.outtake();
        })).onFalse(new InstantCommand(() -> {
            endEffector.stopIntake();
        }));
        ...
        // Autonomous Commands
        auxCircle.whileTrue(new RunCommand(() -> { // [Hold] AutoAim
            pivot.autoAimPivot();
        }, pivot)).onFalse(new InstantCommand(() -> {
            pivot.setAngleDegree(0);
        }));
        ...
        // Climb Commands
        auxDPADUP.onTrue(new InstantCommand(() -> { // [Hold] Climber Up
            climber.climbUP();
        })).onFalse(new InstantCommand(() -> {
            climber.stopClimb();
        }));

        auxDPADDOWN.onTrue(new InstantCommand(() -> { // [Hold] Climber Down
            climber.climbDown();
        })).onFalse(new InstantCommand(() -> {
            climber.stopClimb();
        }));

        auxL2.onTrue(new InstantCommand(() -> { // [Hold] Amp Flywheel
            endEffector.ampFlywheel();
        }, endEffector)).onFalse(new InstantCommand(() -> {
            endEffector.stopFlywheel();
            endEffector.stopIntake();
        ...
        driverCircle.onTrue(new InstantCommand(() -> { // Zero Pivot
            pivot.zeroPivot();
        }, pivot));
        ...
        driverSquare.whileTrue(new RunCommand(() -> { // [Hold] AutoAim
            pivot.autoAimPivot();
        }, pivot)).onFalse(pivot.pivotMoveCommand(PivotConstants.kPivotZero));

        // Shouldn't be used
        driverTriangle.onTrue(new InstantCommand(() -> { // Disable Pivot
            pivot.disable();
        }, pivot)).onFalse(new InstantCommand(() -> { // Enable Pivot
            pivot.enable();
        }, pivot));
    }
